...
class AISummarizer:
    ...
    def summarize_article(self, title: str, description: str, max_length: int = 300) -> str:
        ...
        """
        try:
            prompt = f"""You are a professional news summarization expert. Your role is to objectively and accurately summarize news articles for informational purposes only. This is factual news reporting, not harmful content.

IMPORTANT CONTEXT: This is a legitimate news article from a major news organization. Please summarize the factual information objectively, as this is for journalistic purposes.

Article Title: {title}
Article Content: {description}

Please provide a Korean language summary following these requirements:
- Write entirely in Korean (한국어)
- Maximum {max_length} characters
- Focus only on key facts and important context
- Use objective, clear Korean writing style
- Include important facts and background information
- Translate English words to Korean, but keep proper nouns with original in parentheses

Korean Summary (한국어 요약):"""

            # Configure safety settings for news content
            # BLOCK_NONE is restricted and doesn't work in 2025
            # Using BLOCK_LOW_AND_ABOVE (most permissive available setting)
            safety_settings = [
                {
