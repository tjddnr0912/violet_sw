...
## 입지 enrichment: 신고가 단지 지하철 카운트가 전부 0 (코드 버그 오인)

- **증상**: `location_enrich` 프로토타입 첫 검증에서 신고가 6개 단지 **지하철(SW8) 카운트가 일제히 0**. 미금역 인근으로 보이는 분당 무지개3단지까지 0이라 데이터가 아니라 버그처럼 보임.
- **원인**: 버그 아님. 카카오 SW8 로직 정상(known-positive 강남역 좌표 대조 시 2건 반환). 무지개3단지 최근접 역(오리역)이 실제 **500~1000m 사이**라 500m 반경에선 진짜 0. "미금역 근처일 것"이라는 미검증 지리 가정이 틀렸고 500m가 빡빡한 것.
- **해결**: 코드 수정 불필요. 반경(`LOCATION_ENRICH_RADIUS`)은 설계상 500m 유지. 배지에 `🚇 500m 내 역 없음`으로 정직하게 표기.
- **복구 절차**: (a) known-positive 좌표(강남역)로 SW8 로직 배제 (b) 같은 단지 radius 500 vs 1000 비교로 데이터 여부 확정 (c) 필요 시 반경 env로 조정.
- **관련 사고**: 2026-07-18.
- **재발 감지**: 반경 카운트가 균일 0/동일값이면 코드 버그로 단정 말 것 — 먼저 known-positive 대조 좌표로 로직을 배제하고 반경을 넓혀 데이터인지 확인.

> Claude 진단 미스: **초기 가설이 "지하철 전부 0 = 시스템 버그"였으나, 강남역 대조 + radius 500/1000 비교로 정상 데이터(반경이 빡빡)임이 밝혀짐.** 방향 전환 지점 = 강남역 좌표로 SW8을 직접 호출한 Bash 직후. 교훈: 위치 기반 카운트가 일제히 0/균일하면 (1) **known-positive 대조 좌표로 로직부터 배제** (2) 반경 확대로 데이터 확인 — 지리적 직관("역 근처일 것")은 좌표+반경으로 검증하기 전엔 가정일 뿐. (지하철 로직은 정상, 진짜 남은 문제는 지오코딩 커버리지였음.)
