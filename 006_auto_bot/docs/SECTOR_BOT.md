...
|---------|-------|-------------|
| `GEMINI_MODEL` | gemini-3.1-flash-lite | Gemini 모델 (env `SECTOR_GEMINI_MODEL`) |
...
| 에러 | 처리 |
|------|------|
| Gemini API 429 할당량 초과 | wrapper가 모델 fallback chain으로 자동 전환 (3.1-flash-lite → 3.5-flash → 3-flash-preview → 2.5-flash) |
...
1. **Primary**: `GEMINI_MODEL` 환경변수 (기본 `gemini-3.1-flash-lite`)
2. **Fallbacks**: `GEMINI_FALLBACK_MODELS` 환경변수 (기본 `gemini-3.5-flash,gemini-3-flash-preview,gemini-2.5-flash`)
