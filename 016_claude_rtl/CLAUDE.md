...
>
> **현재:** **2766 테스트 green** · **format_version 19** · MsgCode 57 · 3-OS CI green.
> - **🟢 최신 — wildcard equality `==?`/`!=?` §11.4.6 + 코어 `==`/`!=` definite-mismatch 수정(2026-07-02, `feat-wildcard-eq`, ROADMAP §4.5.62, IR-0·`.vu` 재핀)**: 렉서 토큰+AST `WildEq/WildNe`+**const-pattern mask&compare desugar**(`(lhs&mask)==cleaned`=기존 Eq 재사용·`CasexEq`는 EITHER-side wildcard라 함정·extension 비트=compared·honest-loud=런타임/compound-const pattern). 적대 R1 differential이 **HIGH pre-existing 동반 발굴=코어 `==`/`!=` x-poison**(`4'b1x00==4'b0000`=x여야 0·§11.4.5 ambiguous만 x)→**3사이트 수정**(log_eq+VM EqNe/WEqNe)·옛-동작 인코딩 테스트 0건. soundness CLEAN(인터셉트 완전성·ir_bits_of↔width.rs 동일공식). 14 테스트.
> - **직전 2건(2026-07-02)**: pre-opened FD §21.3.4(§4.5.61=stdout/stderr 라우팅·`$fclose` no-op·E 리포트=docs-undersell 재분류) · `%t`/`$timeformat` full §21.3.2+proc_multipliers u64(§4.5.60=CRITICAL M-포화 동반수정·probe-harvest 후보 9종).
