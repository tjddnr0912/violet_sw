...
>
> **현재:** **2753 테스트 green** · **format_version 19** · MsgCode 57 · 3-OS CI green.
> - **🟢 최신 — pre-opened FD STDIN/STDOUT/STDERR §21.3.4(2026-07-02, `feat-preopened-fd`, ROADMAP §4.5.61, 엔진-only IR-0)**: `$fwrite/$fdisplay(32'h8000_0001/2)`가 W4022+**출력 드롭**이던 것을 → STDOUT=`$display`와 같은 결정적 싱크(문장순서 interleave·골든-안전)·STDERR=프로세스 stderr·`$fclose`(pre-opened)=warn+no-op(계속 사용)·read=write-only 룰(fgetc -1·feof 0 무경고)·STDIN read=deferred(byte-결정성). 적대 R1 양 렌즈 **first-round CLEAN**(differential 12-probe·soundness call-site 전수+fd 충돌 불가 증명). 7 테스트. **동반 재분류: 외부 리포트 E(file-I/O)=docs-undersell 실측 확정**(`$fopen/$fgets/$fscanf` 전부 동작·iverilog byte-exact)→D docs sync로 편입.
> - **직전 — `%t`/`$timeformat` full §21.3.2 + proc_multipliers u64(§4.5.60, IR-0·format 19 불변)**: `%t`=plain-`%0d` broad silent-wrong→완전 구현(severities-선례 StmtId 사이드카=bump 회피·정수 truncation·real %f 라운딩·X/Z collapse·zero-fill·iverilog byte pin 21 테스트). 적대 R1이 **CRITICAL pre-existing u32 M-포화**(`1s/1ps`서 `$time` off-by-decades) 동반 발굴→u64 확장(postcard varint wire-호환 핀). R2 CLEAN. probe-harvest 후보 9종 ROADMAP 기록.
