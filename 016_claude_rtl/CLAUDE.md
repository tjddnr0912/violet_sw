...
>
> **현재:** **2746 테스트 green** · **format_version 19** · MsgCode 57 · 3-OS CI green.
> - **🟢 최신 — `%t`/`$timeformat` full §21.3.2 완성 + proc_multipliers u64(2026-07-02, `feat-timeformat`, ROADMAP §4.5.60, 전부 IR-0·format 19 불변)**: fresh-probe 35-sweep이 발굴한 ① broad silent-wrong — `%t`=plain `%0d`(rescale·기본폭 20 부재)·`$timeformat`=W3056 skip → severities/assert_ctl 선례(no-op Display+StmtId 사이드카=SysTaskId 추가 0=bump 회피)로 런타임 문 구현(last-wins·0 args=리셋·1~3 args=loud·변수 args OK). 렌더=정수 자릿수-문자열+**truncation**(iverilog 핀)·real=%f 라운딩·X/Z collapse+textual zeros·`%0Nt` zero-fill·minw<0=좌측정렬·suffix %s-coercion. hostile caps(units global±64·prec 64·minw 4096). **적대 R1+R2**: differential이 `%0Nt` zero-fill drop 수정·soundness가 **CRITICAL pre-existing `proc_multipliers` u32 포화**(`1s/1ps`=M 10^12→`$time`부터 off-by-decades) 발굴→**u64 확장**(postcard varint=old artifact wire-호환·핀 테스트)+deferred-assert action `$timeformat`=loud+suffix 경로 통일. 21 테스트(iverilog byte pin·staged parity). 부수 발굴 후보 9종(pre-opened FD·`%-` 플래그·queue slice·`==?`·force part-sel·계층 fn call 등)=ROADMAP §4.5.60 기록.
> - **직전 배치(2026-06-29)**: 🏁 외부 사용자 리포트 §6 ② 전량 완료(**A1** 헤더 import·**A3** 모듈 port typedef·**B1** `$value$plusargs`-if·**B2** typedef cast·**C1** SIGPIPE no-panic = §4.5.55~59·A2 array param=defer-deep ROADMAP §6) · string element indexing `s[i]`(§4.5.54) · 🏁 typedef-recognition family 완결(§4.5.40~53). 잔여=D docs sync·E file-I/O TB.
