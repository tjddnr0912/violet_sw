...
- **Phase 2**: 전 항목 — `$test$plusargs`, `$value$plusargs`, `$system`,
  `$fatal`, `$error`, `$warning`, `$info`, `$exit`
...
- **$system**: Rust `std::process::Command`로 셸 호출. 반환값은 exit status.
  시뮬레이션-only 컨텍스트에서만 허용; RTL 내부 호출은 시뮬레이터 에러 발생.
...
- **$exit**: program block 완료 이벤트 채널 구독 → 모두 완료 시 `$finish` 호출.
  Verilator 대상 코드베이스에서는 `$finish` 직접 사용을 권장.
