...
- ✅ **구현됨**: `$test$plusargs`, `$value$plusargs`(format 코드 파서 + ref-VAR 쓰기 statement-level 인터셉트, format_version 7), `$fatal`, `$error`, `$warning`, `$info`(severity 태스크 — `$fatal`=exit1, elaboration/simulation 양 컨텍스트).
- ⏳ **미구현**: `$system`·`$exit`.
  ...
  - `$exit`은 `program` 블록 미지원에 묶여 미배선이며, 태스크 형태라 unknown-task 경고 후 스킵된다. module-based TB는 구현된 `$finish`를 직접 사용한다.
...
> **vitamin 구현 상태**: `$exit`은 `program` 블록 미지원에 묶여 미배선이다.
> 태스크 형태라 unknown-task 경로에서 `W-ELAB-FEATURE-LIMIT` 경고 후 스킵된다(no-op).
> module-based TB에서는 구현된 `$finish`를 직접 사용한다.
...
- **$exit** ⏳ (미구현): `program` 블록 미지원에 블록됨 — 태스크 형태라 unknown-task 경로에서 `W-ELAB-FEATURE-LIMIT` 경고 후 스킵된다. 향후 program block 완료 이벤트 채널 구독 → 모두 완료 시 `$finish` 호출 설계. 현재는 module-based TB에서 `$finish` 직접 사용.
