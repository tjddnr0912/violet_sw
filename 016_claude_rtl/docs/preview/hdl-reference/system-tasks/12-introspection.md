...
- ⏳ **전 함수 미구현 (loud-reject E3009)**: `$typename`, `$cast`, `$isunbounded`,
  `$size`, `$left`, `$right`, `$low`, `$high`, `$increment`,
  `$dimensions`, `$unpacked_dimensions` — 어느 함수도 `SysFuncId`에 배선되지 않아
  elaborate가 `E3009 "unsupported system function"`으로 거부한다.
  ($cast의 class 형태는 OOP(N7) 트랙, 나머지 array-query/타입 introspection은 별도 후속 트랙.)
...
> ⏳ **전 함수 현재 미구현 (loud-reject E3009)** — 아래는 향후 설계 메모다.
> `$cast`(class 형태)는 class/OOP(N7) 트랙에, array-query 계열은 배열 디스크립터
> 인프라 도입 후 합류한다.
