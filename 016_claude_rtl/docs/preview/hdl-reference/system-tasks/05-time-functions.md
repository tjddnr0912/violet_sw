...
- **미구현 (deferred)**: `$stime` — `map_sysfunc`에 미등록, 사용 시 `VITA-E3009 unsupported
  system function`로 loud-reject. 본 페이지의 `$stime` 항목은 IEEE 표준 레퍼런스이며 vitamin
  지원 표기가 아니다. 하위 32비트가 필요하면 `$time & 32'hFFFF_FFFF`로 대체.
...
> **vitamin: 미구현.** 아래는 IEEE 표준 레퍼런스다. 호출 시 `VITA-E3009`로 거부된다
> (위 "지원 Phase" 참조).
...
- `$stime`: **미구현** (`map_sysfunc` 미등록 → `VITA-E3009`). 위 항목 참조.
