...
| # | 결정 | 내용 |
|---|---|---|
| D1 | 전용 크레이트 | (역)직렬화 · 헤더 · 버전 · staleness · `--dump` 로직은 전용 크레이트 `vita-artifact`가 보유. 형상 해시는 proc-macro 크레이트 `vita-artifact-derive`가 산출. 워크스페이스 11→13. `cli`는 얇은 글루로 유지. |
...
SimIr 루트 1개:
  ...
  - process (트리거/감지 집합 + 본문), continuous assign
...
expand(path, base_mode):
  canon = lexical_normalize(resolve(path, base_mode, invocation_cwd))   # symlink 해소 안 함
  if canon ∈ active_stack: error E-FLIST-CYCLE (체인 출력)
  push canon
...
> timeprecision)` + `--timescale` 기본값이며, **velab 합성 해시의 일부**다. `vita`가 staged
...
| `-E` | 전처리만 하고 정지 (view/short-circuit) | — (해시·동치 모델 밖) |
| `-o <work-dir>` | 출력 디렉터리 명명 | C |
