...
SimIr 루트 1개:
  ...
  - process (트리거/감지 집합 + 구조화 basic-block 본문 + 예약된 재개 상태: resume_pc/locals/join_state/wake_key — 06 "프로세스 실행 모델"), continuous assign
...
> **프로세스 실행 모델 = basic-block PC 상태기계 (06 결정 2026-06-01).** `process` 본문은 바이트코드
> ISA가 아니라 **구조화된 basic-block 시퀀스**이며, 멈춘 프로세스의 재개 상태(`resume_pc:u32` /
> `locals:Vec<FourState>` / `join_state` / `wake_key`)는 sim-ir 스키마에 **물리적으로 예약**한다. 나중에
> 추가하면 SCHEMA_HASH가 뒤집혀 모든 기존 `.velab`가 무효화되기 때문이다(§5 RULE D2). 재개 위치는 정수
> 인덱스(포인터·네이티브 PC 금지), locals는 순서 `Vec` — §5 "순서 안정 구조만" 불변식과 3-OS 바이트
> 동일성을 충족한다. fork-join 인코딩·`disable` 서브트리 teardown·wait 포함 task call-frame은 sim-ir
> freeze 전 확정할 하위 결정이다.
