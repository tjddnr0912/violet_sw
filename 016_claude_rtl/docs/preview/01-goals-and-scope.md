...
| 절차 블록 | `initial`, `always`, `always_ff`/`always_comb`/`always_latch`, fork-join + **`join_any`/`join_none`/`disable fork`**(*P2-E 2026-06-12*)·**`wait fork`**(*v8 2026-06-14 — 직계 자식 배리어*), **`final`**(*P2-E — 타이밍 컨트롤=loud*), named `event`/`->`/`@(ev)` | automatic/recursive frame-call |
...
| 타이밍 | `#delay`(상수+**런타임 식** — format_version 4), `@(event)`, `wait`(테스트벤치), intra-assignment `= #d`(실semantics)·`<= #d`(transport, v5)·**repeat-event blocking `= [repeat(n)]@(ev)`**(*`cfef719` — iverilog 차분*) | clocking block, repeat-event NBA(`<= @(ev)`) 형 |
