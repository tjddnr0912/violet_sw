...
- **커버리지/UVM** (N5+) · **`class`/OOP·`program`·`union`** (N7) · **math transcendentals**(`$ln`/`$log10`/`$exp`/`$sqrt`/삼각 — N6, pure-Rust libm 3-OS 결정성 핀 대기로 loud).
...
|---|---|---|
| 설계 단위 | `module`/`endmodule`, 포트, `parameter`/`localparam`, `generate`/`genvar`, `interface`/`modport`(*2026-06-11 IN 승격*), **`package`/`import`/`pkg::`**(*2026-06-12 IN 승격, v7 — param/enum-label/func/task 평탄화; 패키지 변수·패키지내 import는 loud*), **계층 read-only 이름 참조**(*✅ 구현 2026-06-18 N3/N3.1 — `tb.dut.x`·`dut.mem[i]`·`dut.grid[i][j]`·packed `dut.pm[i]` 식-내 read(IEEE §23.6 commit-to-scope, deferred-ref+arena-patch); 계층 WRITE·3-seg indexed·indexed-segment parse(`g[0].x`)·interface-member array element·`$dumpvars` hier arg·hier param은 loud*) | `program`, `class`(N7) |
