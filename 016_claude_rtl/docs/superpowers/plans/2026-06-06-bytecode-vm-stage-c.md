...
| **C3** Native value registers | P12a | ⏳ Perf harness DONE (`tests/perf_baseline.rs`, `#[ignore]`d). **MEASURED FINDING (2026-06-06, reshapes C3):** an A/B of inline-≤128 `Value` storage vs the `Vec<u64>` heap rep showed **~0 net change** (eval-heavy best-of-5: 2802 vs 2781 ms interp; 2714 vs 2699 ms VM — within noise) → **`Value` heap-alloc is NOT the bottleneck** (doc-18's premise contradicted by measurement; tiny short-lived `Vec<u64>` are cheap). The inline experiment was REVERTED (no benefit = pure complexity, YAGNI). Also measured: **the C2 VM is already 0.97x interp on eval-heavy** — it pre-compiles the body and avoids `run_process`'s per-block `stmt_ids/term/stmt` **clones** (exec.rs:88-104). ⇒ The real C3 lever is **removing the eval tree-walk dispatch (native eval) + per-statement clone overhead**, NOT the value rep. **Next: profile to pin the dominant cost before the native-eval reimplementation** (C4-level width/sign/poison correctness gates it anyway). | P5 + P12a bit-exact |
