...
- **다음 결정 후보**(스코프 결정 영역, 작업 큐 아님): full SVA(시퀀스 `##n`/repetition/throughout=관찰 오토마타·multi-clock·disable iff·action block·module-level 문법) · class/OOP·`program`·union(검증 생태계) · 커버리지 · automatic/recursive 콜스택 모델 · math transcendentals(순수-Rust libm 핀).
