...
| **N3** | hierarchical 이름 참조(읽기 한정) | `tb.dut.x`를 식에서 참조(현 E3009). 인스턴스 경계 넘는 read-only 해소(symbols FQ 경로). 멀티모듈 TB 흔함. 중간 공수·iverilog 오라클. | ⭐ **High(다음)** — 흔한 TB 패턴·iverilog 강한 오라클 |
