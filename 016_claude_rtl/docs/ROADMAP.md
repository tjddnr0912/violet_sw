...
6. **full SVA 시퀀스 (사용자 스코프=오토마타 트랙까지, 多슬라이스 진행 중)**:
   ...
   - **S8+ 잔여 (오토마타-FSM, ⭐IR-0 tractable로 정정)**: `[->n]` goto·`[=n]` nonconsec·`within`·`[*m:$]` unbounded repeat(mid). ⭐**정정**: `|->`는 *any*-completion만 필요(exact count 불요)→**existence-latch FSM**(스테이지당 n boolean state reg, goto: `p0'=cur|(p0&~b)`, `p1'=(b&p0)|(p1&~b)`, match=`b&p1`)로 순수 IR-0 합성 가능(이전 "single-bit 머지 불가→infeasible" 우려는 exact-count 한정 오판). AST flip(repeat-kind 또는 신규 variant) 동반. sequence consequent·multi-clock·disable iff·action block·module-level 문법도 잔여.
