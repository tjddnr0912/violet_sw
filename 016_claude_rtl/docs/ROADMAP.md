...
> **갱신:** 2026-06-08 (native-eval C4-lite 랜딩 + 멀티-top 다중 root elaborate; 직전 2026-06-07 C7 timescale 버그 + 문서부채). 현 단계 = **Stage C 컴파일드 백엔드 C1·C2 완료**(VM MVP, byte-identical) + **native-eval C4-lite(식-바운드 VM 0.42x)** +
...
1. **native-eval 연산자 커버리지 확장 (§C follow-on)** — 비교/시프트/Div·Mod/리덕션/ternary/select·concat을 native 서브셋에 추가(각 P5 게이트로 byte동일 강제). 식-바운드 적용폭 확대. 점진·중위험.
2. **perf 트랙 (워크로드 분기):**
   - 클럭-바운드 목표면 → **스케줄러 축** (`sched.rs` event wheel·`propagate_changes`·NBA).
   - >64bit/real native lane (C6) — signed-lane·heap fallback. 고위험.
3. **언어 커버리지 (§D)** — 시뮬레이터 유용성 확장 (perf 아님).
4. **저빈도 perf 잔여 (§A)** — value-op word化·미세 최적화. 낮은 ROI·저위험.
