...
6. ~~**full SVA 시퀀스 슬라이스 S4**~~ ✅ **2026-06-15 완료(945 green, `c4efbd5` push)** — 사용자 선택(검증 트랙 진행). 상수 `##n` cycle-delay + `[*n]` consecutive repetition을 antecedent에서 desugar(`flatten_sequence`→`synth_seq_pipeline` 시프트레지스터, 단일-term=flat byte-identical, fmt_ver 8 유지·`.vu` 재핀만). 관문=GO/IR-0 판정(iverilog bare `|->`조차 거부 재확인=전면 hand-IEEE). 적대 리뷰 4렌즈 CLEAN, MEDIUM 1건(`[*` 토큰이 wildcard `[*]` 진단 degrade) 수정. §4.1 P2-F 행 참조.
- **다음 결정 후보**(스코프 결정 영역, 작업 큐 아님): **SVA 시퀀스 후속 슬라이스**(범위 `##[m:n]`/`[*m:n]`=parallel-pipeline OR(IR-0, blowup) → unbounded `[*0:$]`/`[->n]`/`[=n]`=관찰 오토마타 트랙(大) → throughout/within·sequence consequent·multi-clock·disable iff·action block·module-level 문법) · class/OOP·`program`·union(검증 생태계) · 커버리지 · automatic/recursive 콜스택 모델 · math transcendentals(순수-Rust libm 핀).
