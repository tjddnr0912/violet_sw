...
- **그 외 결정 후보**(스코프 결정 영역): class/OOP·`program`·union(검증 생태계) · 커버리지 · automatic/recursive 콜스택 모델 · math transcendentals(순수-Rust libm 핀) · SVA action block의 sampled-value·local var · named property/sequence **formal arguments**(필드 reserve 완료) · multi-clock(S15 두-프로세스).
