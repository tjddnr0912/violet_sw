...
**판단(2026-06-10 갱신):** 세 축 모두 1라운드 수확 완료 — value word化 ~6x(eval-heavy) · 스케줄러축 ≈1.85x(클럭-바운드) · native-eval ≈2.4~2.8x(식/구조-바운드 VM). **잔여 perf 후보(ROI 순): ①dirty-list 넷 스캔**(스케줄러 라운드2 — per-delta O(nets) 스캔이 대형 디자인 병목; 쓰기 훅+정렬로 byte-identity) **②native >64bit/real/array-indexed lane**(C6 — 멀티워드 레지스터 파일, 고위험) ③저빈도 value-op word化·미세최적화(위 표 1·2행).
...
1. **스케줄러 라운드 2: dirty-list 넷 스캔** — `propagate_changes`의 per-delta O(nets) `cur != prev` 전수 스캔을 쓰기-훅 dirty 리스트로(정렬+dedup으로 byte-identity 유지). 넷 수천 개 디자인의 다음 병목. 중규모.
2. **filelist typed 버킷** — `+incdir+`/`+define+`를 PreOpts에 배선(현재 PreOpts::default() — `-D`/-I 표면 자체가 없음) + `E-FLIST-WRONG-STAGE`/`W-FLIST-OVERRIDE`. 실프로젝트 유용성 직결.
