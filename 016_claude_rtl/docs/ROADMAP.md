...
| **N3.2** | LOCAL array-of-packed sub-element read/write (`qm[i][j]`) | pre-existing silent 1-bit(N3.1-followon 적대 리뷰 적발): `reg [3:0][7:0] qm[0:1]`의 `qm[i][j]`가 read·write 모두 packed byte 아닌 1-bit. local `lower_array_read`/`collect_array_write`가 surplus index를 packed-dim flatten 아닌 single bit로 처리. iverilog 차분. 작음(IR-0). | **Medium** — 정확성 핀(read+write 쌍) |
