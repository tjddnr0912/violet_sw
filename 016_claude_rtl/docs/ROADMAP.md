...
6. **full SVA 시퀀스 (사용자 스코프=오토마타 트랙까지, 多슬라이스 진행 중)**:
   ...
   - **S9+ 잔여**: `within`(NFA product/intersect=最난이도·드뭄)·`[*m:$]` unbounded repeat(mid, gated run-latch)·sequence consequent·multi-clock·disable iff·action block·module-level 문법.
