...
6. **full SVA 시퀀스 (사용자 스코프=오토마타 트랙까지, 多슬라이스 진행 중)**:
   ...
   - **S7+ 오토마타-hard(잔여)**: `[*m:$]` unbounded repeat(gated run-latch)·`[->n]` goto·`[=n]` nonconsec(per-thread 카운터)·`throughout`(window AND, IR-0 expand)·`within`(NFA product) — single-bit 머지 불가=진짜 오토마타, AST flip 동반 예상. sequence consequent·multi-clock·disable iff·action block·module-level 문법도 잔여.
