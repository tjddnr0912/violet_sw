...
| **N2** | SVA 잔여(multi-term cross-clock·고급 prop-ref skew·sequence local var·recursive property) | ✅ **N2a-1**(`e804546`, multi-clock seq antecedent `@(c1) a ##1 @(c2) b |-> c`) · ✅ **N2b**(`9fbb02d`, 2-cycle non-overlap prop-ref skew) · ✅ **N2d**(2026-06-17, property-level `and`/`or` + recursive property: `PropExpr` 트리→per-clock viol 환원, 순수 IR-0, always/weak-until idiom; 적대 4렌즈→**HIGH silent-wrong 수정**=skew 추적(混합-skew `and`/`or`=loud)+parser `[*`/`[->`/`[=` depth 카운트+nesting depth cap) · ⏸ **N2c**(sequence local var = per-attempt thread storage가 thread-collapsing 파이프라인과 근본 상충 → honest loud reject 유지) · ▶ **N2a-2**(N-clock chains·\|=>cross-seq·multi-term seg) 잔여. | ✅ 대부분 소진(N2a-2만 잔여) |
