...
| ~~`assert property`~~ ✅ P2-F(SVA 서브셋, Phase-3) · `assert #0`/`final` 연기형 assertion · clocking block · repeat-event intra-assign 고급 (loud) | ~~P2-F~~ — 잔여 컷만 후속 |
...
- **그 외 결정 후보**(스코프 결정 영역): class/OOP·`program`·union(검증 생태계) · 커버리지 · automatic/recursive 콜스택 모델 · math transcendentals(순수-Rust libm 핀). **SVA 트랙 A1–A4 전부 완료**; 잔여 SVA(별개·후속)=multi-term 시퀀스 cross-clock(A3 확장)·outer-`|=>` prop-ref skew 합성·action block의 sampled local var·sequence local variable·recursive property.
