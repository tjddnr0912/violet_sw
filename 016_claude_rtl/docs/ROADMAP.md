...
> **갱신:** 2026-06-08 (멀티-top 다중 root elaborate 수정 반영; 직전 2026-06-07 C7 timescale 버그 + 문서부채). 현 단계 = **Stage C 컴파일드 백엔드 C1·C2 완료**(VM MVP, byte-identical) +
...
2. **언어 커버리지 (§D)** — 시뮬레이터 유용성 확장 (perf 아님).
3. **저빈도 perf 잔여 (§A)** — value-op word化·미세 최적화. 낮은 ROI·저위험.
