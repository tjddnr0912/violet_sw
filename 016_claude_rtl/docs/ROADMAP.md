...
| **N3.1** | hierarchical 배열/멀티딤 element read (`dut.mem[i]`) | N3 후속(리뷰 적발): `dut.mem[i]`/`dut.pm[i]` 식-내 read(현 loud). DUT 메모리 TB read=흔함, **iverilog 강한 오라클**. hierarchical indexed read를 element-select 경로로 라우팅(resolved net 기반). 중간 공수·정확 구현 가능. | ⭐ **High(다음)** — 흔한 TB 패턴·강한 오라클·정확구현 가능 |
