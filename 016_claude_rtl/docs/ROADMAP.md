...
**판단(2026-06-10 갱신):** 세 축 모두 1라운드 수확 완료 — value word化 ~6x(eval-heavy) · 스케줄러축 ≈1.85x(클럭-바운드)+dirty-list R2(NETS_HEAVY ≈19.7x) · native-eval ≈1.4~2.8x(식/구조/wide/mem-바운드 VM, C6 포함). **잔여 perf 후보(ROI 순): ①저빈도 value-op word化·미세최적화(위 표 1·2행) ②wide 구조 트리오·real native lane(좁은 영역, 저ROI).** *(C6 array-indexed + 128-bit lane ✅ 2026-06-10 — doc-18 §실측. net_to_edge/waiter 다넷 후보는 `perf_nets_scaling` 프로브(512→8192 평탄)로 **무근거 판명·폐기** — 같은 날.)*
