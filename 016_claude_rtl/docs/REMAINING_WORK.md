...
- [x] **[가속·로드맵]** 컴파일드 백엔드 (IR→네이티브 코드젠, Verilator 방식) — 📋 문서화된 장기 로드맵. **⚠ 2026-06~07 SUPERSEDED:** 바이트코드 VM으로 **C1·C2 실구현**(아래 Stage C 섹션) + **프로파일링이 "코드젠만이 10~100× 가속" 전제를 정정** — 진짜 병목은 bit-serial 처리(코드젠 무관, 공유 경로 word化/inline으로 누적 ~6x), eval 트리워크(native-eval 대상)는 ~2-4%뿐. **native-eval은 저ROI로 내려놓음.** 향후 = [`ROADMAP.md`](ROADMAP.md), 실측 = doc-18 §실측.
...
| **P8** ✅ | 커널-콜 **순서/샘플링-모먼트 불변식 계약** 동결 | 🔴 B | P7a→P7b | 2026-06-06. doc-18 §P8에 7 모먼트 동결 + **분류**: VM 능동책임은 #2(NBA 순서)·#3(blocking offset)뿐(문장 텍스트순 실행으로 자동충족, StmtEffect가 #3 캡처); #1/#5=인터프리터-only(cont-assign), #4=non-codegen(in-body @), #6/#7=커널측(공유). corpus `cont_mixed`(연속할당+delayed+클럭 혼재) 추가로 #1/#5 커버. 433 green |
...
| **P7b** ✅ | body↔kernel **trait/ABI seam 도입**(코드젠 가능化 커밋) | 🔴 B | P7a,P9 | 2026-06-06. exec.rs `trait Kernel`(k_eval_for_lvalue/k_resolve_lvalue_offsets/k_write_lvalue/k_schedule_nba/k_dispatch_systask — frozen sim-ir 타입 verbatim). `compute_effect`/`apply_effect`를 `<K:Kernel>` 제네릭化 → **인터프리터가 이미 seam 통과**(전 suite가 증명). `impl Kernel for Scheduler`(인헌런트 위임). suspend/resume·fork는 **부재**(P9 suspend-free class 전용; suspend 바디는 인터프리터 잔류). truthy/rearm 터미네이터면은 Stage C vm_run서 추가. 433 green, P5 byte동일 |
...
### Stage C — 컴파일드 바이트코드 VM (C1·C2 ✅ + profile-driven perf ~6x ✅ · 다음 = [ROADMAP](ROADMAP.md))

> **요약(2026-06-07):** C1·C2(VM MVP, byte동일) 완료 후 **profile→fix→re-profile 4R로 누적 ~6x**(eval-heavy 2781→461ms). 모든 윈이 interp·VM **공유 경로**(net I/O·shift·resize word化 + inline-Value + read/mask 정리). **프로파일링이 doc-18의 eval=병목 전제를 정정 → native-eval(plan C3~C6) 저ROI로 내려놓음.** 아래는 increment별 시계열 기록(이력). **향후 과제·전략 결정·우선순위 = [`ROADMAP.md`](ROADMAP.md), 예측-vs-측정 = doc-18 §실측.**

> **📋 스펙/플랜 ✅ + 리뷰 ✅ (2026-06-06):** `docs/superpowers/plans/2026-06-06-bytecode-vm-stage-c.md`. 9단계(C1~C9, 각 단계 P5 byte동일 검증). 적대적 리뷰(`w7a0hlp1z`, 4렌즈→REVISE)의 6 must-fix(cur_time_mult·delta-guard를 C2로 이동, truthiness=k_truthy/X·Z→false, borrow protocol+template-keyed cache, C2=구조적 마일스톤·C3가 첫 가속, k_rearm(proc) invariant) + 10 should-fix 전부 반영.
>
> **🔨 C1·C2 구현 완료 (2026-06-06, MVP terminus):** **C1** — `Kernel` trait에 터미네이터/제어 ABI 4 포워더(`k_truthy`/`k_rearm`/`k_max_deltas`/`k_mark_fatal`, exec.rs) 추가. **C2** — 레지스터 바이트코드 VM(backend.rs `CompiledBody`/`Op`/`compile_body`/`vm_exec`): 1:1 BB lowering(P16 매핑), out-of-band template-keyed `vm_cache`(decide-once, state.rs `VmSlot`/`vm_compiled` — borrow protocol으로 `Rc` owned handle), **cur_time_mult 프롤로그**(must-fix #1)+**per-activation delta guard**(must-fix #2)를 `vm_run_body`/`vm_exec`서 재현. 코드젠 가능 바디는 이제 **실제로 VM 실행**(인터프리터 위임 제거) — P5 차분 게이트가 corpus 72설계+mixed(P9b) byte동일 강제. 신규 teeth: 컴파일패스 단위테스트 2(블록/터미네이터 1:1·NBA/systask shape) + 프롤로그(distinct M `$time`, 적대 검증으로 teeth 확인)·runaway 가드(`forever`→Fatal)·blocking-index(`a[i]=K;i=i+1` P8#3) 3. **439 green, clippy/fmt clean, golden(SimIr) unflipped, frozen IR 무변경.**
>
> **🔬 C3 착수 — perf 하네스 + 측정 기반 negative 결과 (2026-06-06):** `tests/perf_baseline.rs`(`#[ignore]`, DATA-not-gate) 추가. **측정 발견: inline-≤128 `Value` 저장 vs `Vec<u64>` 힙 rep A/B = ~0 변화**(eval-heavy best-of-5: interp 2802 vs 2781ms, VM 2714 vs 2699ms — noise 내) → **`Value` 힙할당은 병목 아님**(doc-18 전제가 측정으로 반증; 작은 단명 `Vec<u64>` alloc은 저렴). inline 실험 **revert**(이득 0 = 순수 복잡도, YAGNI). 부수 발견: **C2 VM이 eval-heavy서 이미 0.97x interp** — 바디 선컴파일로 `run_process` 블록당 `stmt/term` **clone**(exec.rs:88-104) 회피. ⇒ value rep 아님.
>
> **🔬🔬 C3 프로파일링 — plan 전제 전복 (2026-06-06, `/usr/bin/sample`, eval-heavy debuginfo):** self-time 히스토그램이 **C3 방향 자체를 뒤집음**. **net-write 경로가 ~50% 지배** — `write_lvalue` 5707 + `set_vu` 3970 + `slice_word` 1392 + `resize`/`mask_top` 844 — 게다가 **bit-serial**(`write_lvalue`/`slice_word`/`write_chunk`가 `get_vu`/`set_vu`/`bit_of`/`set_bit`로 1비트씩 루프 + chunk마다 `Value::zeros` alloc). malloc/free ≈ ~17%. **eval 트리워크 디스패치(`eval_ctx`)는 단 ~1.5%**, 전 eval value-ops ~15%. ⇒ **native eval(C3-C6)은 최고위험 재구현으로 ~1.5–15% 추격; 진짜 ~50% 윈 = word-parallel net read/write**(state.rs `write_lvalue`/`write_chunk`/`slice_word` + value.rs `set_vu`/`get_vu` + `bit_of`/`set_bit`) — contained·저위험·**양 backend 공통 이득**(VM이 eval로 못 앞지름). **피벗 → word-parallel net write.**
>
