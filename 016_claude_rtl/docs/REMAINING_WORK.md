...
- [ ] **[P-perf] `decimal_bits` O(n²) — 거대 십진 리터럴 parse DoS** (pre-existing, P0-10 리뷰가 표면화·diff 무관). `literal.rs:99` schoolbook base-10→binary 장제법이 비트당 O(digits) 패스라 O(digits²): 40000자리=27s, 50000자리=43s(iverilog 0.01s 평탄). 폭 캡(1M bit≈315k digit)은 결과 메모리만 바운드, parse 시간은 미해소. 진짜 해소=near-linear 변환(divide-and-conquer big-int) 또는 digit-count 게이트. 실 RTL에는 무영향(수백 비트 ≤ ~100자리=µs), 적대 입력 한정.
