...
>
> **✅ B2 코어 완료(2026-06-23, rejection-sampling 솔버, format_version 11)**: inter-variable(`x<y`·`a+b==50`)·`x inside {set/[lo:hi]}`·implication `a->b`·`soft` constraint — `COp` 포스트픽스 술어 바이트코드(non-frozen, 골든 무영향). B2 적대 hunt 8 confirmed 수정(randomize() §18.11 반환값·signed>64bit·wide-술어 i64-unsafe loud). **B2 잔여(각 별개 슬라이스)**: `dist`(가중 sampling)·`randc`(cyclic 순열상태)·inline `randomize() with {…}`(per-call·UserTaskCall AST flip)·`rand` real/string/array 멤버. **별개 pre-existing**: `int/byte/shortint/longint unsigned`가 range_to_dims에서 signed 취급(tri-state signedness 필요).
