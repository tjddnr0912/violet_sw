...
check_directory() {
    if [[ ! -f "main.py" ]] || [[ ! -f "trading_bot.py" ]]; then
        echo -e "${RED}❌ 005_money 디렉토리에서 실행해주세요.${NC}"
        echo "   필요한 파일: main.py, trading_bot.py"
        exit 1
    fi
...
check_config() {
    ...
    python -c "import config; print('✅ 설정 파일 로드 성공')" || {
        echo -e "${RED}❌ 설정 파일 로드 실패${NC}"
        exit 1
    }

    # API 키 확인
    API_CONFIGURED=$(python -c "import config; print(config.BITHUMB_CONNECT_KEY != 'YOUR_CONNECT_KEY')" 2>/dev/null)
