...
class TransactionHistory:
    ...
    def get_summary(self, ticker: str = None, days: int = None) -> Dict[str, Any]:
        ...
        # Calculate net P&L from sell transactions
        net_pnl = sum(t.get('pnl', 0) for t in filtered_transactions if t['success'])

        # Count failed transactions
        fail_count = len([t for t in filtered_transactions if not t['success']])
