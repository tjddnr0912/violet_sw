...
def check_config(python_cmd):
    ...
    try:
        result = subprocess.run([
            python_cmd, "-c",
            "import config; print('✅ 설정 파일 로드 성공'); print('API Keys configured:', config.BITHUMB_CONNECT_KEY != 'YOUR_CONNECT_KEY')"
...
def run_trading_bot(main_args):
    ...
    try:
        # main.py에 인수 전달
        cmd = [python_cmd, "main.py"] + main_args
...
def main():
    ...
    if run_args.test_api:
        ...
        python_cmd = get_python_command()
        try:
            subprocess.run([python_cmd, "test_api_connection.py"], check=True)
        except subprocess.CalledProcessError:
            print("❌ API 테스트 실행 실패")
        return
