...
## 2026-04-07: KIS API EGW00103 토큰 자동 갱신

- **EGW00103 자동 복구**: `kis_client.py`의 `_request()`에서 `EGW00103` (유효하지 않은 AppKey) 응답 감지 시 토큰 강제 갱신 후 1회 재시도
  - KIS 모의투자 서버 일시 장애 시 자동 복구
  - `token_refreshed` 플래그로 무한 루프 방지
...
### 버그 수정
- **실행 실패 추적**: `_execute_order` 실패(Rate Limit 등) 시 `failed_orders`에 이관되지 않던 문제 수정
- **Silent Drop 방지**: `quantity=0`(투자금 < 주가) 시 로그 없이 폐기되던 주문을 `failed_orders`에 추가 + 경고 로그
- **사유 오분류 수정**: `_check_position_shortage`가 모든 미달을 "스크리닝 미달"로 표시 → "주문 실행 실패" / "주문 생성 실패" / "기타"로 세분화
- **에러 로그 개선**: `_execute_buy` 에러 로그에 종목명/코드 추가

### API Rate Limit 대응
- `API_DELAY_VIRTUAL`: 0.5초 → 1.0초 (모의투자 초당 1건 제한 대응)
- `API_DELAY_REAL`: 0.1초 → 0.15초 (KIS 권장 최소 간격)
