...
| 016_claude_rtl | **vitamin** — 오픈소스 Rust RTL 시뮬레이터 (SystemVerilog/Verilog, cargo 워크스페이스). 전 파이프라인 동작 + Phase-3 SVA 서브셋(multi-clock·recursive property·named/formal) + deferred immediate asserts + frame-call 콜스택(automatic/recursive) + **HIER-REST 완결**(계층 read+write·part-select·whole packed·`g[0].x`) + **functional coverage 완성(N5: bins·iff·cross·option.at_least/weight·real-% 가중평균·`@(clk)` auto-sample)** (1516 tests green, format_version 9) | `cargo test --workspace --locked` | [CLAUDE.md](016_claude_rtl/CLAUDE.md) |
