...
## 데일리 포트폴리오 요약에 trend sleeve의 BIL(안전자산)이 안 보임 (버킷 값이 첫 종목만 표시)

- **증상**: ET 00:00(KST 13:00) 데일리 틱 포트폴리오 요약에서 trend 버킷이 `TQQQ`만 표시되고 함께 보유 중인 `BIL`(vol-target 안전자산 leg)이 누락. 동시에 trend 평가금액이 실제보다 작게(예: $524 vs 실제 TQQQ+BIL $616), drift가 비정상적으로 큰 음수(예: −18% — 실제 ~−4%)로 표시. broker 계좌엔 BIL이 멀쩡히 들려 있음.
- **원인**: `src/core/portfolio.py` `evaluate_portfolio`의 `_bucket_value("trend"/"tqqq_sma")`가 `(TQQQ, BIL)`을 돌며 **qty>0인 첫 종목만 반환**하고 즉시 return. vol-target sleeve는 노출에 따라 TQQQ(asset)+BIL(safe)을 **동시 보유**하는데, 첫-매치가 항상 TQQQ를 잡아 BIL leg 평가금액을 통째로 누락 → 버킷 값 과소·drift 왜곡·BIL 불가시.
- **해결**: 두 브랜치를 병합해 (TQQQ, BIL) **둘 다 합산**하고 라벨도 `"TQQQ+BIL"`로 결합 반환. notifier Symbol 컬럼 폭 6→9(divider 47→50)로 결합 라벨 정렬 보존. 신규 테스트 2건(양쪽 보유 합산 / BIL-only). **표시 전용 — 매매 로직 무관**(trend는 자체 월간 스케줄로만 리밸런스, `needs_rebalance`가 trend 제외). (2026-06-04)
- **복구 절차**: (a) 코드 반영 후 **봇 재시작**(실행 중 프로세스엔 구코드가 메모리에 떠 있음). (b) 재시작 전까진 `portfolio_state.json`의 trend 버킷 값이 stale(BIL 누락분)로 남고, 다음 데일리 틱 `save_evaluation`에서 합산값으로 갱신. (c) 실보유는 항상 broker 기준 → 자산 자체엔 영향 없음(cosmetic).
- **관련 사고**: 2026-06-04 (사용자가 "13시 요약에 BIL이 왜 안 보이나" + "거래가능금액 ~$110은 의도된 건가" 질문 → 로그 `2026-06-01 23:36 BUY BIL x1 @ $92.31` 확인 → BIL은 보유 중이고 표시만 누락으로 확정·수정)
- **재발 감지**: 요약 trend 행 Symbol이 `TQQQ` 단독이고 drift가 비정상 큰 음수인데 broker 보유종목엔 BIL이 있으면 재발. `_bucket_value`가 TQQQ/BIL 중 하나만 반환하도록 회귀했는지 점검. ($10k 티어에서 `trend`+`tqqq_sma`가 공존하며 둘 다 TQQQ/BIL에 매핑 → 별개의 이중계상 가능성은 미해결 백로그, 현 자본 티어엔 무해.)

### Claude 진단 미스 (2026-06-04)

- **Claude 처음 가설**: 거래가능금액 ~$110이 trend sleeve의 BIL(안전자산 16.7%≈$107) 배분이 **매수되지 못하고 현금으로 남은 것**이라 추정 — 즉 "BIL을 안 사서 요약에도 안 뜬다". $107≈$110 수치 일치에 끌림.
- **실제 원인**: BIL은 **정상 매수·보유 중**(로그 `2026-06-01 23:36 BUY BIL x1 @ $92.31`, 이후 매도 0). 요약 누락은 `_bucket_value` 첫-매치 표시 버그 탓. ~$110 현금은 BIL과 **별개** — 5개 포지션 정수주 반올림 + 매수 시 1.25%(슬리피지+수수료) 버퍼 미소진 + 분기/월말까지 미리밸런스 drift의 잔여(세 버킷 타깃 미달분 합).
- **방향 전환 지점**: 최근 로그를 grep해 `BUY BIL x1 @ $92.31`을 본 직후. 그 전까진 $107≈$110 일치를 근거로 "미배치 cash 가설"에 무게.
- **교훈 (다음에 'ETF가 요약/버킷에 안 보임'을 보면)**:
  - 첫 의심 영역: **"안 산 게 아니라 표시에서 빠진 것"** 부터 — broker fills 로그(`grep "BUY <SYM>" logs/app/`)와 잔고로 실보유를 먼저 확정한 뒤 표시 로직(`_bucket_value` 첫-매치 vs 합산)을 본다.
  - 빨리 배제할 가설: "잔여 현금 = 특정 sleeve leg 미배치". 현금은 보통 **여러 버킷의 반올림·버퍼·drift 합**이지 한 leg의 미집행이 아니다. 수치 우연 일치($107≈$110)에 과신 금지.
  - 핵심 진단 명령: `grep -rn "BUY \|SELL " logs/app/` (실제 체결) + `portfolio_state.json` 버킷 값 vs broker 보유 대조.
