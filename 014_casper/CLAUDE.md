...
KIS 토큰 lockout (EGW00103) / cold-start HTTP 500 / .env IFS trailing-byte / 잔고 API 분기 / 포지션 사이징 vs limit price mismatch / 매도 fill buffer / orphan 포지션 / 더블 매도 / 테스트 prod 데이터 누수 / trend·GEM 월말 리밸런스 미실행(RTH 보류) — 각 항목은 6필드 + Claude 진단 미스 기록 구조로 → [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md).
