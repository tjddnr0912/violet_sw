...
KIS 토큰 lockout (EGW00103) / cold-start HTTP 500 / .env IFS trailing-byte / 잔고 API 분기 / 포지션 사이징 vs limit price mismatch / 매도 fill buffer / orphan 포지션 / 더블 매도 / 테스트 prod 데이터 누수 / trend·GEM 월말 리밸런스 미실행(RTH 보류) / 메인 루프 CPU 100% busy-loop / trend 요약 BIL 표시 누락(버킷 첫-종목만) — 각 항목은 6필드 + Claude 진단 미스 기록 구조로 → [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md).

**가장 흔한 함정 1줄 가이드**:
- `EGW00103` 보면 → 키 의심 X, **토큰 retry 빈도 의심 O**
- KIS HTTP 500 + 빈 body → 서버 장애 X, **secret 길이 비교 O** (`echo "SEC_LEN=${#KIS_APP_SECRET}"`)
- **trend/GEM이 안 나간다** → 스케줄러·키 의심 X, **데일리 틱이 미국장 마감(ET 00:00=KST 13:00)에 떨어져 auto 실행이 보류됐는지 의심 O**. `data/trend_state.json`의 `last_signal_date`가 비고 trend 버킷이 $0면 이 케이스. RTH 재개는 `_gem_pending`/`_trend_pending`가 처리(2026-06-01 fix) — 봇 재시작이면 충분
- **봇 CPU 100% 상시** → 키·시장 무관, **메인 루프 sleep 위치 의심 O** (`run()`의 sleep이 정상 경로에도 있는지). collector/`select.so`는 보통 헛다리. 확인: `ps -o time,etime,%cpu -p <PID>` (CPU시간≈경과시간이면 busy-loop). 2026-06-02 fix로 `_loop_iteration()`이 매 틱 sleep
